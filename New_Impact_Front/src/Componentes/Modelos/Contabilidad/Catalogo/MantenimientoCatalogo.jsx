...
const MantenimientoCatalogo = () => {
  ...
  const rutaNuevo = `/CatalogoNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Catalogos";
  const rutaEditar = `/CatalogoEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "CatalogoEditar"
...
const getLista = (codigo_compania) => {
  ...
  CatalogoServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setCatalogo(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const eventoConfirmar = () => {//si la variable eliminar no es null invoca el metodo eliminar
  ...
  const remove = (IDPais) => { 
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
      ...
      CatalogoServicios.remove(IDPais)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
      ...
      CatalogoServicios.setAuthToken(token);
        ...
        CatalogoServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
              .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    ...
    { field: 'Permite_Sub_Cuentas',                headerName: 'Permite Sub Cuentas'},
    { field: 'Nombre_Cuenta',                      headerName: 'Nombre de Cuenta'},
    { field: 'Nombre_Idioma',                      headerName: 'Idioma'},
    { field: 'Descripcion',                        headerName: 'Descripción'},
    { field: 'Fecha_Ultimo_Movimiento',            headerName: 'Fecha del Último Movimiento'},
    { field: 'Requiere_Origen_Destino',            headerName: 'Requiere Origen y Destino'},
    { field: 'Requiere_Control_Presupuestario',    headerName: 'Requiere Control Presupuestario'},
    { field: 'Permite_Transacciones',              headerName: 'Permite Transacciones'},
    { field: 'IDContabilidad_Tipo_cuenta',         headerName: 'Tipo de cuenta'},
    { field: 'IDContabilidad_Niveles',             headerName: 'Nivel contable'},
  ...
  useEffect(() => {
    const datosCompletoTemp = catalogo.map(objecto => ({
      id:                               objecto.IDContabilidad_Catalogo ,
      IDOrigen:                         objecto.IDOrigen,
      Cuenta_Formateada:                objecto.Cuenta_Formateada,
      Cuenta:                           objecto.Cuenta,
      Permite_Sub_Cuentas:              objecto.Permite_Sub_Cuentas ? 'Sí' : 'No',
      Nombre_Cuenta:                    objecto.Nombre_Cuenta,
      Nombre_Idioma:                    objecto.Nombre_Idioma,
      Descripcion:                      objecto.Descripcion,
      Fecha_Ultimo_Movimiento:          objecto.Fecha_Ultimo_Movimiento,
      Requiere_Origen_Destino:          objecto.Requiere_Origen_Destino ? 'Sí' : 'No',
      Requiere_Control_Presupuestario:  objecto.Requiere_Control_Presupuestario ? 'Sí' : 'No',
      Permite_Transacciones:            objecto.Permite_Transacciones ? 'Sí' : 'No',
      Visible:                          objecto.Visible,
      IDCompania:                       objecto.IDCompania.IDCompania,
      IDContabilidad_Tipo_cuenta:       objecto.IDContabilidad_Tipo_cuenta.Detalle,
      IDContabilidad_Niveles:           objecto.IDContabilidad_Niveles.Nivel,
...
export default MantenimientoCatalogo;