...
const MantenimientoNiveles = () => {
  ...
  const rutaNuevo = `/NivelesNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Niveles contables";
  const rutaEditar = `/NivelesEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "NivelesEditar"
...
const getLista = (codigo_compania) => {
  ...
  NivelesServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setNiveles(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
useEffect(() => {
  ...
  const remove = (IDPais) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
     ...
     NivelesServicios.remove(IDPais)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
        ...
        NivelesServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
              .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    { field: 'Nivel',           headerName: 'Nivel'},
    { field: 'Nombre_Nivel',    headerName: 'Nombre del Nivel'},
  ...
  useEffect(() => {
    const datosCompletoTemp = niveles.map(objecto => ({
      id:                 objecto.IDContabilidad_Niveles ,
      Nivel:              objecto.Nivel,
      Nombre_Nivel:       objecto.Nombre_Nivel,
...
export default MantenimientoNiveles;