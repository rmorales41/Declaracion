...
const MantenimientoDatafonos = () => {
  ...
  const rutaNuevo = `/DatafonosNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Datafonos";
  const rutaEditar = `/DatafonosEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "DatafonosEditar"


//Guarda el código de la compañía en la constante setCodigo_compania
useEffect(()=>{
...
const getLista = (codigo_compania) => {
  ...
  DatafonosServicios.setAuthToken(token); // Le manda el token al encabezado de la consulta del endpoind
  DatafonosServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setDatafonos(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const remove = (id) => {
  const token = AuthServices.getAuthToken()// Trae el token que se gurdo en el localStores cuando se logueo
    ...
    DatafonosServicios.remove(id)//Invoca el endpoid de elimanar o remove del back end
      .then((response) => {
...
const EliminarConCheck = (seleccionadosEliminados) => {
  ...
  DatafonosServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
    .then(() => {
...
const [encabezadosCompleto] = useState([
  { field: 'Identificador',                   headerName: 'Identificador'},
  { field: 'Comision',                        headerName: 'Comisión'},
  { field: 'IDContabilidad_Cuentas_Bancarias',           headerName: 'Cuenta bancaria'},
  { field: 'IDConfiguracion_Bancos',          headerName: 'Banco'},
  { field: 'IDPlanilla_Funcionarios',         headerName: 'Funcionario'},
...
useEffect(() => {
  const datosCompletoTemp = datafonos.map(objecto => ({
      id                    : objecto.IDContabildiad_Datafonos,
      Identificador         : objecto.Identificador,
      Comision              : objecto.Comision,
      IDCompania            : objecto.IDCompania.IDCompania,
      IDContabilidad_Cuentas_Bancarias       : objecto.IDContabilidad_Cuentas_Bancarias ? objecto.IDContabilidad_Cuentas_Bancarias.Cuenta_Numero : null,
      IDConfiguracion_Bancos        : objecto.IDConfiguracion_Bancos ? objecto.IDConfiguracion_Bancos.Descripcion : null,
      IDPlanilla_Funcionarios       : objecto.IDPlanilla_Funcionarios ? objecto.IDPlanilla_Funcionarios.Nombre : null,
...
export default MantenimientoDatafonos;