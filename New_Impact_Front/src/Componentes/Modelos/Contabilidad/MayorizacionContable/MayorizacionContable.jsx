...
const MayorizacionContable = () => {
  const { idFormulario } = useParams();

  const InicializaMayorizacionContable = {
    fecha_mayorizacion: "",
    codigo_compania:"",
  ...
  const [mayorizacionContable, setMayorizacionContable] = useState(InicializaMayorizacionContable);
  ...
  useEffect(()=>{
    ...
    setMayorizacionContable(prevAsiento => ({...prevAsiento, codigo_compania: codigo_companiaAuth}));
  },[])
...
const eventoObtenerFecha = async (date, name) => {
  if (!date || isNaN(date)){//Valida que la fecha no venga vacia, y que venga en el formato correcto
      setMayorizacionContable(prevAsiento => ({...prevAsiento, [name]: null}));
    ...
    if(respuesta ){
      setMayorizacionContable(prevAsiento => ({...prevAsiento, [name] : formatoDeFecha}));
      setError(null)
    }else{
      setMayorizacionContable(prevAsiento => ({...prevAsiento, [name]: null}));
      setError("Seleccione una fecha válida.")
...
const realizarMayorizacionContable = async () => {
  ...
  if(!mayorizacionContable.fecha_mayorizacion){
  ...
  try {
   ...
   const response = await  MayorizacionContableServicios.mayorizacionContable(mayorizacionContable.codigo_compania, mayorizacionContable.fecha_mayorizacion); // Invoca el método listar o el get de todo de servicios
...
useEffect(() => {
  const contabilidad_Confi = async () => {
    ...
    if (configuracionConta && configuracionConta.length > 0) {
      setMayorizacionContable(prevAsiento => ({...prevAsiento,
...
const Advierte_Fecha_Final_Dia_Mayorizacion_y_Cierre = (fecha_accion_contable) => {
  ...
  const { Fecha_ultimo_cierre, Cierre_Contable } = mayorizacionContable;
...
export default MayorizacionContable;