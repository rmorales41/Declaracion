...
const MantenimientoTipoAsiento = () => {
  ...
  const rutaNuevo = `/TipoAsientoNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Tipos de Asientos";
  const rutaEditar = `/TipoAsientoEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "TipoAsientoEditar"
...
const getLista = (codigo_compania) => {
  ...
  TipoAsientoServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setTipoAsiento(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const eventoCancelar = () => {
  ...
  const remove = (id) => {
    ...
    TipoAsientoServicios.remove(id)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
        ...
        TipoAsientoServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
              .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    //{ field: 'IDContabilidad_Tipo_Asiento',  headerName: 'IDContabilidad_Tipo_Asiento'},
  ...
  useEffect(() => {
    const datosCompletoTemp = tipoAsiento.map(objecto => ({
        id                : objecto.IDContabilidad_Tipo_Asiento,
        Descripcion       : objecto.Descripcion,
        Indicador         : objecto.Indicador,
