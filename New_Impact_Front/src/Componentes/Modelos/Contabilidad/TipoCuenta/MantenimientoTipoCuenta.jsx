...
const MantenimientoTipoCuenta = () => {
  ...
  const rutaNuevo = `/TipoCuentaNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Tipos de cuentas contables";
  const rutaEditar = `/TipoCuentaEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "TipoCuentaEditar"
...
const getLista = (codigo_compania) => {
  ...
  TipoCuentaServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setTipoCuenta(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
useEffect(() => {
  ...
  const remove = (IDPais) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
      ...
      TipoCuentaServicios.remove(IDPais)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
        ...
        TipoCuentaServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
              .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    ...
    { field: 'Origen_Destino',     headerName: 'Origen y Destino'},
  ...
  useEffect(() => {
    const datosCompletoTemp = tipoCuenta.map(objecto => ({
      id:                objecto.IDContabilidad_Tipo_cuenta ,
      Identificador:     objecto.Identificador,
      Detalle:           objecto.Detalle,
      Origen_Destino:    objecto.Origen_Destino ? 'Requiere Origen y Destino' : 'No Requiere Origen y Destino',
...
export default MantenimientoTipoCuenta;