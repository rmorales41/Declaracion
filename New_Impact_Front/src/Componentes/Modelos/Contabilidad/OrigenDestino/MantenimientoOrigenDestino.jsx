...
const MantenimientoOrigenDestino = () => {
  ...
  const rutaNuevo = `/OrigenDestinoNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Origenes y Destinos";
  const rutaEditar = `/OrigenDestinoEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "OrigenDestinoEditar"
...
const getLista = (codigo_compania) => {
  ...
  OrigenDestinoServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setOrigenDestino(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const eventoCancelar = () => {
  ...
  const remove = (IDPais) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
     ...
     OrigenDestinoServicios.remove(IDPais)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
        ...
        OrigenDestinoServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
              .then(() => {
  ...
  useEffect(() => {
    const datosCompletoTemp = origenDestino.map(objecto => ({
      id:                objecto.IDContabilidad_Origen ,
      Codigo:            objecto.Codigo,
      Descripcion:       objecto.Descripcion,
      Estado:            objecto.Estado ? 'Activo' : 'Inactivo',
...
export default MantenimientoOrigenDestino;