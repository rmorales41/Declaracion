...
const MantenimientoCuentasBancarias = () => {
  ...
  const rutaNuevo = `/CuentasBancariasNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Cuentas Bancarias";
  const rutaEditar = `/CuentasBancariasEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "CuentasBancariasEditar"
...
const getLista = (codigo_compania) => {
  ...
  CuentasBancariasServicios.setAuthToken(token); // Le manda el token al encabezado de la consulta del endpoind
  CuentasBancariasServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setCuentasBancarias(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const remove = (id) => {
  const token = AuthServices.getAuthToken()// Trae el token que se gurdo en el localStores cuando se logueo
    ...
    CuentasBancariasServicios.remove(id)//Invoca el endpoid de elimanar o remove del back end
      .then((response) => {
...
const validaSeleccionadosEliminar = () => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    CuentasBancariasServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
      .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    { field: 'IDConfiguracion_Bancos',    headerName: 'Bancos'},
    { field: 'Cuenta_Numero',    headerName: 'Numero de cuenta'},
    { field: 'Iban',             headerName: 'Iban'},
    { field: 'Cheque',           headerName: 'Cheque'},
    { field: 'Moneda',           headerName: 'Moneda'},
    { field: 'Tiene_Datafonos',      headerName: 'Datáfonos'},
    { field: 'IDCuenta_Contable',    headerName: 'Comisiones'},
    { field: 'IDCuentasxCobrar_Banco',    headerName: 'Cuentas por cobrar banco'},
    { field: 'Cuenta_Formateada',   headerName: 'Cuenta'},
  ...
  useEffect(() => {
    const fetchDatosCompleto = async () => {
      const datosCompletoTemp = await Promise.all(
        cuentasBancarias.map(async (objecto) => {
          let cuentaContableFormateada = null;
          if (objecto.IDCuenta_Contable) {
            const cuentaContable = await CatalogoHooks.obtenerCuentaFormatiada(objecto.IDCuenta_Contable);
            cuentaContableFormateada = cuentaContable?.Cuenta_Formateada || null;
          }
          let cuentaxCobrarFormateada = null;
          if (objecto.IDCuentasxCobrar_Banco) {
            const cuentaContablexCobrar = await CatalogoHooks.obtenerCuentaFormatiada(objecto.IDCuentasxCobrar_Banco);
            cuentaxCobrarFormateada = cuentaContablexCobrar?.Cuenta_Formateada || null;
          }

          return {
            id                    : objecto.IDContabilidad_Cuentas_Bancarias,
            Cuenta_Numero         : objecto.Cuenta_Numero,
            Iban                  : objecto.Iban,
            Cheque                : objecto.Cheque,
            Moneda                : objecto.Moneda ? 'Local' : 'Extranjera',
            Tiene_Datafonos       : objecto.Tiene_Datafonos ? 'Si' : 'No',
            IDCuenta_Contable     : cuentaContableFormateada,
            IDCuentasxCobrar_Banco    : cuentaxCobrarFormateada,
            IDContabilidad_Catalogo   : objecto.IDContabilidad_Catalogo ? objecto.IDContabilidad_Catalogo.Cuenta_Formateada : null,
            IDConfiguracion_Bancos    : objecto.IDConfiguracion_Bancos.Descripcion,
            IDCompania                : objecto.IDCompania.IDCompania,
            Cuenta_Formateada         : objecto.Cuenta_Formateada,
...
return (
  ...
  {/*Propiedades que se le pasa al componente <ContenedorTabla />*/}
    <ContenedorTabla
        ObjetoMapeado={cuentasBancarias}
        rutaNuevo={rutaNuevo}
...
export default MantenimientoCuentasBancarias;