...
import FormularioCuentasBancarias from "./FormularioCuentasBancarias";
...
useEffect(() => {
  ...
  setCuentaBancarias({ ...cuentaBancarias, IDCompania: codigoCompaniaAuth});
...
const buscarPor = async (id, codigo_compania) => {
  ...
  setCargando(true); // Muestra el componente de cargar

  try {
    CuentasBancariasServicios.setAuthToken(token);
    const response = await CuentasBancariasServicios.findBy(id, codigo_compania); // Invoca el endpoint del backend
    const respuesta = response.data;
    setExistenteOriginal(respuesta.Cuenta_Numero);
    ...
    const dataModificada = { // Modifica solo los campos necesarios y deja los demás igual, esto se hace mas que nada para los SelectAutoCompleteFormulario
      ...respuesta, 
      IDContabilidad_Catalogo: respuesta.IDContabilidad_Catalogo?.IDContabilidad_Catalogo || null,
      IDConfiguracion_Bancos: respuesta.IDConfiguracion_Bancos?.IDConfiguracion_Bancos || null,
    };

    setCuentaBancarias(dataModificada);
  } catch (e) {
    ValidaRestricciones.capturaDeErrores(e); 
  } finally {
    setCargando(false); // Oculta el componente de cargar
  }
};
        ...
        .then((confirmed) => {
            if(confirmed){// Si se confirma se modifica el registro
              var data = { // Le agrega todos los datos a la variable data
                ...
                IDCompania                            : cuentaBancarias.IDCompania.IDCompania,
              };
                setCargando(true); // Activa el componente de carga
                CuentasBancariasServicios.setAuthToken(token);
                CuentasBancariasServicios.update(cuentaBancarias.IDContabilidad_Cuentas_Bancarias, data)//Invoca el endpoid del backend 
                    .then(response => {
                      setCargando(false)
...
const validarExistentes = (validar) => {
  ...
  listaCuentaBancarias.forEach(listaforEach => {// Se hace un forEach de la variable 
    if (listaforEach.Cuenta_Numero === validar) {// Busca en el Array la variable si ya existe 
      if(existenteOriginal !== validar) errores.Nombre = "El número de cuenta: " + validar +" ya existe. Por favor, ingrese uno diferente"; // Si ya existe en el array se guarda un mensaje para mostrarlo como error
    }
  })
...
return(
  <>
    ...
    <FormularioCuentasBancarias {...propsParaFormulario} />
...
export default CuentasBancariasEditar;