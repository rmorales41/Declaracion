...
import TipoDeCambiosServicios from '../../../../Servicios/ConfiguracionServicios/TipoDeCambioServicios/TipoDeCambioServicios';
...
const MantenimientoTipoCambio = () => {
  ...
  const rutaNuevo = `/TipoDeCambioNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Tipo de Cambio";
  const rutaEditar = `/TipoDeCambioEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "TipoDeCambioEditar"
...
const getLista = (codigo_compania) => {
  ...
  TipoDeCambiosServicios.setAuthToken(token); // Le manda el token al encabezado de la consulta del endpoind
  TipoDeCambiosServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setTipoDeCambio(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const eventoCancelar = () => {
  ...
  const remove = (IDPais) => {
    ...
    TipoDeCambiosServicios.setAuthToken(token);
    TipoDeCambiosServicios.remove(IDPais)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    setCargando(true)// Acá, llama o invoca el componente Loading Aleatorio, poniento la const en true
        ...
        TipoDeCambiosServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
              .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    ...
    { field: 'Compra',         headerName: 'Compra'},
    { field: 'Venta',          headerName: 'Venta'},
    { field: 'Personalizado',  headerName: 'Personalizado'},
  ...
  useEffect(() => {
    const datosCompletoTemp = tipoDeCambio.map(objecto => ({
        id:                objecto.IDConfiguracion_Tipo_Cambio ,
        Fecha:             objecto.Fecha,
        Compra:            objecto.Compra,
        Venta:             objecto.Venta,
        Personalizado:     objecto.Personalizado ,
        Usuario:           objecto.Usuario,
        Fecha_Proceso:     objecto.Fecha_Proceso.IDCompania,
...
export default MantenimientoTipoCambio;