...
const MantenimientoTipoAsiento = () => {
  ...
  const rutaNuevo = `/BancosNuevo/${idFormulario}`;//La ruta para el formulario de agregar un nuevo registro
  const tituloPagina = "Bancos";
  const rutaEditar = `/BancosEditar/${seleccionadosEliminados}/${idFormulario}`;//La ruta para el formulario de editar un registro
  const moduloEditar = "BancosEditar"
...
const getLista = (codigo_compania) => {
  ...
  BancosServicios.setAuthToken(token); // Le manda el token al encabezado de la consulta del endpoind
  BancosServicios.getAll(codigo_compania) // Invoca o llama el metodo listar o el get de todo de servicios  
      .then(response => {
        setBancos(response.data); // Guarda lo que se devolvió del back-end en la variable que es un array
...
const remove = (id) => {
  const token = AuthServices.getAuthToken()// Trae el token que se gurdo en el localStores cuando se logueo
    ...
    BancosServicios.remove(id)//Invoca el endpoid de elimanar o remove del back end
              .then((response) => {
...
const validaSeleccionadosEliminar = () => {
 ...
 const EliminarConCheck = (seleccionadosEliminados) => {
    ...
    BancosServicios.EliminarConCheck(data)// Invoca el endpoid de elimanar o remove del back end
      .then(() => {
  ...
  const [encabezadosCompleto] = useState([
    ...
    { field: 'Estatal',             headerName: 'Estatal'},
    { field: 'Pago_Automatico_Salarios',     headerName: 'Pago automático de salarios'},
    { field: 'Correo',               headerName: 'Correo'},
    { field: 'Contacto',             headerName: 'Contacto'},
    { field: 'Codigo_Bancario',      headerName: 'Código Bancario'},
    { field: 'Codigo_Empresa',       headerName: 'Código de Empresa'},
  ...
  useEffect(() => {
    const datosCompletoTemp = bancos.map(objecto => ({
        id                : objecto.IDConfiguracion_Bancos,
        Descripcion       : objecto.Descripcion,
        Estatal           : objecto.Estatal ? "Estatal" : "Privado",
        Pago_Automatico_Salarios  : objecto.Pago_Automatico_Salarios ? "Si" : "No",
        Correo                    : objecto.Correo,
        Contacto                  : objecto.Contacto,
        Codigo_Bancario           : objecto.Codigo_Bancario,
...
export default MantenimientoTipoAsiento;